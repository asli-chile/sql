-- SOLUCIÓN DEFINITIVA CON HISTORIAL COMPLETO
...
BEGIN
  ...
  IF current_auth_id IS NULL THEN RETURN NULL; END IF;
  
  SELECT id INTO usuario_id FROM usuarios WHERE usuarios.auth_user_id = current_auth_id;
  IF usuario_id IS NOT NULL THEN RETURN usuario_id; END IF;
...
-- 3. CREAR FUNCIÓN DE HISTORIAL COMPLETA SIN AMBIGÜEDADES
...
SELECT 'SOLUCIÓN DEFINITIVA CON HISTORIAL COMPLETO IMPLEMENTADA' as resultado;
