  ...
  const baseColumns: ColumnDef<Registro>[] = [
    {
      ...
      maxSize: 0,
      enableHiding: true,
      // Esta columna se mantendrá oculta por defecto mediante el estado inicial en RegistrosPage
    },
    {
      id: 'refCliente',
        ...
        return (
          <div className="flex items-center gap-2 w-full">
            <div className="flex-1 flex items-center gap-2">
              {value ? (
                <span className={`px-2 py-1 text-sm font-medium ${isCancelado ? 'bg-red-600 text-black rounded' : ''
                  }`}>
                  {value}
                </span>
              ) : (
            ...
            customDisplay={contenedores.length > 1 ? (
              <div className="flex flex-col gap-1">
                {contenedores.map((container, index) => (
                  <span key={index} className={`px-2 py-0.5 rounded text-[10px] font-mono ${isCancelado ? 'bg-red-600 text-black' : 'bg-gray-100 dark:bg-gray-700 text-black dark:text-gray-200'
                    }`}>
                    {container}
                  </span>
                ))}
